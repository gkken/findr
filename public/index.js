...
function handleLink() {
  currentLocation = new google.maps.LatLng(
    allRandomData.lat.toFixed(5),
    allRandomData.long.toFixed(5)
  );
...
function renderNearest5(nearestStations) {
  return nearestStations
    .filter(station => nearestStations.indexOf(station) < 5)
    .map(renderNearest)
    .join("");
}
...
function updateNearestStations() {
  ...
  axios
    .get(`/api/stations/nearest?lat=${centerLat}&long=${centerLong}&rad=25`)
    .then(res => {
...
function placeMarkers() {
  axios
    .get(
      `/api/stations/nearest?lat=${currentLocation.lat()}&long=${currentLocation.lng()}&rad=15`
    )
    .then(res => {
      res.data.forEach(station => {
        ...
        let withinBound = isWithinBounds(googleCoord);

        if (withinBound && !isMarkerOnMap(googleCoord)) {
